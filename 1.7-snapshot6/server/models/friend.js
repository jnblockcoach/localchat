...
class FriendModel {
  ...
  // 拒绝好友请求（操作者为被请求方，删除 pending 行）
  static rejectRequest(userId, friendId) {
    const result = getDb()
      .prepare("DELETE FROM friends WHERE user_id = ? AND friend_id = ? AND status = 'pending'")
      .run(friendId, userId);
    if (result.changes > 0) {
      logger.info(`好友请求已拒绝: user=${userId} friend=${friendId}`);
      return true;
    }
    return false;
  }
  ...
  // 直接建立双向好友关系（注册 AI 助理时自动与当前用户成为好友）
  static autoFriend(userId, friendId) {
    getDb()
      .prepare("INSERT OR IGNORE INTO friends (user_id, friend_id, status) VALUES (?, ?, 'accepted')")
      .run(userId, friendId);
    getDb()
      .prepare("INSERT OR IGNORE INTO friends (user_id, friend_id, status) VALUES (?, ?, 'accepted')")
      .run(friendId, userId);
    logger.info(`自动好友: ${userId} <-> ${friendId}`);
  }
}
