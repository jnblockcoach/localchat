...
class TerminalUI {
  ...
  async _loginFlow() {
    ...
    try {
      ...
      if (Array.isArray(res) && this.onServerReachable) this.onServerReachable();
    } catch {}
  ...
  _drawStatusBar() {
    ...
    this._put(1, H - 3, '在线:' + online + '  ' + (conn ? '●已连接' : '○断开') + '  Tab切换面板  Ctrl+C退出', 'gray');
  ...
  async _doShowRequests() {
    try {
      ...
      if (!Array.isArray(requests) || requests.length === 0) { this._appendSystemMsg('暂无好友请求'); return; }
