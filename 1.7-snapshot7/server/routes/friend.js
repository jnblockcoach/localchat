...
router.post('/add', requireOwnership((req) => req.body.userId), (req, res) => {
  try {
    ...
    // 目标是 AI 助理账号：直接建立好友关系（无需等待对方接受）
    const target = UserModel.findById(friendId);
    if (target && target.is_ai) {
      FriendModel.autoFriend(userId, friendId);
      return res.json({ request: { status: 'accepted' }, friend: target });
    }
...
router.post('/accept', requireOwnership((req) => req.body.userId), (req, res) => {
  try {
    const { userId, friendId } = req.body;
    if (!userId || !friendId) {
      return res.status(400).json({ error: '缺少参数' });
...
router.post('/reject', requireOwnership((req) => req.body.userId), (req, res) => {
  try {
    const { userId, friendId } = req.body;
    if (!userId || !friendId) {
      return res.status(400).json({ error: '缺少参数' });
    ...
    const ok = FriendModel.rejectRequest(userId, friendId);
    if (!ok) {
      return res.status(404).json({ error: '未找到好友请求' });
    }

    // 通知请求方：请求已被拒绝
    notifyRequestHandled(friendId, userId, 'rejected');
...
router.delete('/', requireOwnership((req) => req.body.userId), (req, res) => {
  try {
    const { userId, friendId } = req.body;
    if (!userId || !friendId) {
      return res.status(400).json({ error: '缺少参数' });
