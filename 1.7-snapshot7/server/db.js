...
function initDatabase() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS ai_peers (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  try { db.exec("ALTER TABLE users ADD COLUMN is_ai INTEGER DEFAULT 0"); } catch {}
  try { db.exec('ALTER TABLE users ADD COLUMN display_id TEXT'); } catch {}

  // AI 账号全局唯一（数据库级约束，防止并发/异常路径重复注册）
  try {
    db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_users_ai_unique ON users(is_ai) WHERE is_ai = 1');
  } catch {}
