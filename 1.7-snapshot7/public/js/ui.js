window.UI = {
  ...
  renderSidebarList(items, type) {
    ...
    list.innerHTML = items
      .map((item) => {
        ...
        const avatar = this.escapeHtml(String(name).charAt(0).toUpperCase());
        const colors = [
          '#e94560',
          '#0f3460',
          '#4ade80',
  ...
  async loadChatView(type, id) {
    // 加载令牌：防止切换对话后，旧对话的异步响应覆盖新对话界面
    const token = ++this._chatLoadToken;
    const stillCurrent = () =>
      this._chatLoadToken === token &&
      this.currentChatType === type &&
      Number(this.currentChat) === Number(id);
    ...
    if (type === 'friend') {
      ...
      this.renderFriendChatHeader(id, isOnline, isBlocked);
    } else {
      try {
    ...
    const cached = CACHE.getMessages(currentUser.id, type, id);
    if (cached.length > 0) {
      if (stillCurrent()) this.renderMessages(cached);
    }

    try {
      let messages;
      if (type === 'friend') {
        messages = await API.getPrivateHistory(currentUser.id, id, 100, currentUser.id);
      } else {
        messages = await API.getGroupHistory(id, 100, currentUser.id);
      }
      if (!stillCurrent()) return;
      CACHE.setMessages(currentUser.id, type, id, messages);
  ...
  renderFileMessage(msg, _side, _time) {
    ...
    const previewUrl = `/api/files/${file.id}/preview?userId=${currentUser.id}`;
    const downloadUrl = `/api/files/${file.id}/download?userId=${currentUser.id}`;
    ...
    if (['jpg', 'jpeg', 'png', 'bmp'].includes(ext)) {
      ...
      if (file.size > 200 * 1024) {
        body = '<div class="file-preview-text-loading" style="color:#888">文件较大 (' + fileSize + ')，请点击 ⬇ 下载查看</div>';
      } else {
        const textContent = this._cachedText[file.id];
        if (textContent) {
          body = `<div class="file-preview-text">${this.renderMarkdown(textContent)}</div>`;
        } else {
          body = `<div class="file-preview-text-loading" data-file-id="${file.id}">加载中...</div>`;
  ...
  async _loadTextPreview(fileId, url) {
    ...
    // 精确选择对应消息的加载占位，避免多条文本消息时错位更新
    const pickEl = () => document.querySelector(`.file-preview-text-loading[data-file-id="${fileId}"]`);
    try {
      const res = await fetch(url);
      if (!res.ok) throw new Error('HTTP ' + res.status);
      const text = await res.text();
      // 缓存上限：循环删除最旧项，防止大量文件撑爆内存
      const keys = Object.keys(this._cachedText);
      while (keys.length >= 50) {
        delete this._cachedText[keys.shift()];
      }
      ...
      this._loadingText.delete(fileId);
    }
  },
  ...
  renderMarkdown(text) {
    ...
    html = html.replace(/\[(.+?)\]\((.+?)\)/g, (m, text, url) => {
      const safe = url.startsWith('http://') || url.startsWith('https://') || url.startsWith('mailto:') || url.startsWith('ftp://');
      return safe ? '<a href="' + url + '" target="_blank">' + text + '</a>' : text + '(' + url + ')';
    });
  ...
  renderMessage(msg) {
    ...
    const highlighted = this.escapeHtml(msg.content)
      .replace(/\n/g, '<br>')
  ...
  showFriendRequests(requests) {
    ...
    // 异步加载 OpenClaw 互联请求（与好友请求同等级展示）
    this.loadAiPeerRequests(body);
  ...
  // 在好友请求弹窗中展示 OpenClaw 互联请求
  async loadAiPeerRequests(body) {
    let peers = [];
    try {
    ...
    wrap.style.cssText = 'border-top:1px solid rgba(255,255,255,0.1);margin-top:12px;padding-top:12px';
    wrap.innerHTML = `<div style="font-weight:bold;margin-bottom:8px">📡 OpenClaw 互联请求</div>` + pending.map((p) => `
      <div class="friend-request-item">
        <div class="friend-request-info">
          <div class="search-result-name">机器 ${this.escapeHtml(p.ip)} 请求互联 OpenClaw</div>
          <div class="search-result-ip">同意后本服务器将使用该机器的 OpenClaw（无法拒绝，只能忽略）</div>
        </div>
        <button class="friend-request-action accept" data-peer-ip="${this.escapeHtml(p.ip)}">同意</button>
      </div>`).join('');
    body.appendChild(wrap);
    wrap.querySelectorAll('[data-peer-ip]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        const r = await API.acceptAiPeer(btn.dataset.peerIp, currentUser.id);
        alert(r.error ? r.error : (r.message || '已同意互联'));
        this.hide('friend-requests-modal');
      });
    });
  },
  ...
  renderFriendChatHeader(friendId, isOnline, isBlocked) {
    const membersEl = this.getEl('chat-members');
    membersEl.innerHTML = `${isOnline ? '🟢 在线' : '🔴 离线'}
      ...
      <button class="block-btn unfriend-btn"
        onclick="APP.deleteFriend(${friendId})">删除好友</button>`;
  },
  ...
  updateOnlineStatus(userId, isOnline) {
    ...
    if (el) {
      ...
      if (this.currentChatType === 'friend' && Number(this.currentChat) === Number(userId)) {
        this.renderFriendChatHeader(userId, isOnline, this.blockedUsers.has(Number(userId)));
      }
    }
  },
  ...
  updateBlockButton(friendId, isBlocked) {
    if (this.currentChatType === 'friend' && Number(this.currentChat) === Number(friendId)) {
      this.renderFriendChatHeader(friendId, this.onlineUsers.has(Number(friendId)), isBlocked);
    }
  },
