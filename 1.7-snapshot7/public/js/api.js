window.API = {
  ...
  async addGroupMember(groupId, userId, opUserId) {
    const res = await fetch('/api/groups/add-member', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ groupId, userId, opUserId }),
    });
    return res.json();
  },

  async removeGroupMember(groupId, userId, opUserId) {
  ...
  async getFileInfo(fileId, userId) {
    const res = await fetch(`/api/files/${fileId}/info?userId=${userId || ''}`);
    return res.json();
  },
  ...
  async getPrivateHistory(userId1, userId2, limit = 100, userId) {
    const res = await fetch(`/api/messages/private/${userId1}/${userId2}?limit=${limit}&userId=${userId || ''}`);
    return res.json();
  },

  async getGroupHistory(groupId, limit = 100, userId) {
    const res = await fetch(`/api/messages/group/${groupId}?limit=${limit}&userId=${userId || ''}`);
    return res.json();
  },
  ...
  async getAiStatus(userId) {
    const res = await fetch('/api/ai/status' + (userId ? '?userId=' + userId : ''));
    return res.json();
  },

  async registerAi(username, registrantId) {
  ...
  async getAiWorkspace(file, userId) {
    const res = await fetch(`/api/ai/workspace${file ? '?file=' + encodeURIComponent(file) : ''}${userId ? (file ? '&' : '?') + 'userId=' + userId : ''}`);
    return res.json();
  },

  async getAiPeers() {
    const res = await fetch('/api/ai/peers');
    return res.json();
  },

  async interconnectOpenClaw(targetIp) {
    const res = await fetch('/api/ai/interconnect', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async acceptAiPeer(targetIp, userId) {
    const res = await fetch('/api/ai/interconnect/accept', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ targetIp, userId }),
    });
    return res.json();
  },

  async rejectAiPeer(targetIp, userId) {
    const res = await fetch('/api/ai/interconnect/reject', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ targetIp, userId }),
    });
