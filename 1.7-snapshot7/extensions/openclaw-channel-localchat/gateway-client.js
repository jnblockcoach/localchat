...
export class GatewayChat {
  ...
  // 发送消息到 agent 会话，异步返回完整回复（同一会话串行执行）
  sendChat({ sessionKey, message }) {
    if (!this.ready || !this.client) return Promise.reject(new Error('Gateway 未连接'));
    const queue = this._sessionQueues.get(sessionKey) || Promise.resolve();
    const task = queue.then(() => this._doSendChat({ sessionKey, message }));
    this._sessionQueues.set(sessionKey, task.catch(() => {}));
    return task;
  }

  async _doSendChat({ sessionKey, message }) {
