...
class Client {
  ...
  getAiStatus(userId) {
    return this.api('/api/ai/status' + (userId ? '?userId=' + userId : ''));
  }
  ...
  getAiWorkspace(file, userId) {
    return this.api(`/api/ai/workspace${file ? '?file=' + encodeURIComponent(file) : ''}${userId ? (file ? '&' : '?') + 'userId=' + userId : ''}`);
  }

  getAiPeers() {
    return this.api('/api/ai/peers');
  }

  interconnectOpenClaw(targetIp) {
    return this.api('/api/ai/interconnect', {
      method: 'POST',
      body: JSON.stringify({ targetIp }),
    });
  }

  acceptAiPeer(targetIp, userId) {
    return this.api('/api/ai/interconnect/accept', {
      method: 'POST',
      body: JSON.stringify({ targetIp, userId }),
    });
  }

  rejectAiPeer(targetIp, userId) {
    return this.api('/api/ai/interconnect/reject', {
      method: 'POST',
      body: JSON.stringify({ targetIp, userId }),
    });
  }
  ...
  rejectFriend(userId, friendId) {
    return this.api('/api/friends/reject', {
      method: 'POST',
      body: JSON.stringify({ userId, friendId }),
    });
  }
  ...
  addGroupMember(groupId, userId, opUserId) {
  ...
  removeGroupMember(groupId, userId, opUserId) {
  ...
  getGroupHistory(groupId, limit = 50, userId) {
    return this.api(`/api/messages/group/${groupId}?limit=${limit}&userId=${userId || ''}`);
  }
