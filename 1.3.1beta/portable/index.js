...
class PortableChat {
  ...
  async run() {
    ...
    if (!Array.isArray(check)) {
      ...
      console.log('  1. 在项目目录 (1.3.1beta) 执行:  npm install');
  ...
  async _openChat(friendId) {
    ...
    try {
      const res = await this.client.getPrivateHistory(this.user.id, friendId, 50);
  ...
  _setupWsHandlers() {
    ...
    this.client.on('friend_online', (data) => {
      ...
      if (this.user && Number(data.userId) === Number(this.user.id)) return;
