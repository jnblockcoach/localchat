...
class TerminalUI {
  ...
  async _showRequestsView() {
    ...
    try {
      ...
      this._aiPeers = (pres && Array.isArray(pres.peers)) ? pres.peers : [];
      this._aiPeersPending = this._aiPeers.filter((p) => p.status === 'pending');
    } catch {
      this._aiPeers = [];
      this._aiPeersPending = [];
    }
  ...
  _renderAiView(errMsg) {
    ...
    // 显示互联 OpenClaw 列表（含已互联的对方机器）
    const peers = this._aiPeers || [];
    let py = 8;
    for (let i = 0; i < Math.min(peers.length, 3); i++) {
      const p = peers[i];
      const st = p.status === 'accepted' ? '● 已互联' : '○ 等待对方确认';
      this._put(cx + 2, cy + py, ' 📡 ' + p.ip + '  ' + st, p.status === 'accepted' ? 'green' : 'yellow');
      py++;
    }
    if (peers.length > 3) this._put(cx + 2, cy + py, ' … 共 ' + peers.length + ' 台', 'gray');
