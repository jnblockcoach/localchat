...
class TerminalUI {
  ...
  async _loginFlow() {
    ...
    for (const u of shownUsers) {
      items.push('  登录 ' + u.username + ' (#' + u.id + ') [' + u.ip + '-' + (u.ip_index || '?') + ']');
    }
  ...
  _renderAiView(errMsg) {
    ...
    const cy = Math.max(0, Math.floor((H - 14) / 2));
    this._box(cx, cy, 56, 14);
    this._put(cx + 2, cy + 1, ' AI 助理   [Enter]注册  [ESC]返回', 'brightCyan');
    this._put(cx + 2, cy + 2, '\u2500'.repeat(52), 'gray');
    ...
    if (acc) {
      this._put(cx + 2, cy + 5, ' 已注册 AI 助理: ' + acc.username + ' (#' + acc.id + ') [' + acc.ip + '-' + (acc.ip_index || '?') + ']', 'yellow');
    } else {
    ...
    this._put(cx + 2, cy + 13, errMsg ? '⚠ ' + errMsg : ' 按 Enter 注册  ESC 返回', errMsg ? 'red' : 'gray');
  }

  // AI 视图按键：Enter 注册流程，ESC 返回
  async _handleAiKey(name) {
    if (name === 'ENTER') {
      ...
      if (st.account) {
        this._renderAiView('AI 助理已注册：' + st.account.username + ' (#' + st.account.id + ')');
        return;
      }
      const W = this._screenW();
      const H = this._screenH();
      ...
      this._renderAiView('✓ AI 助理已注册：' + res.user.username + ' (#' + res.user.id + ')');
      // 注册后自动加为好友，刷新列表
