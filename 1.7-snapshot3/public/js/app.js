...
window.APP = {
    ...
    try {
      ...
      if (users.length > 0) {
        container.innerHTML = '<div class="existing-accounts-label">本机已有账号（点击登录）：</div>' + users
          .map((u) => `<button class="existing-account-btn" data-user-id="${u.id}" data-username="${UI.escapeHtml(u.username)}"><strong>#${u.id}</strong> ${UI.escapeHtml(u.username)} <span style="color:#888;font-size:12px">(${UI.escapeHtml(u.ip)}-${u.ip_index || '?'})</span></button>`)
          .join('');
  ...
  // 注册 AI 助理（手工注册：检测本机 OpenClaw → 设置名称 → 自动加为好友）
  async aiSetup() {
    let status = null;
    try {
    ...
    if (!status.openclaw || !status.openclaw.running) {
      alert('未检测到本机运行的 OpenClaw（请先启动 OpenClaw gateway）');
      return;
    }

    if (status.account) {
      const acc = status.account;
      alert(`AI 助理已注册：${acc.username} (#${acc.id})，可直接在好友列表中找到它`);
      return;
    }

    const name = prompt('检测到本机 OpenClaw，请输入 AI 助理名称：', 'AI助手');
    ...
    alert(`AI 助理已注册：${result.user.username} (#${result.user.id})，已自动添加为好友`);
    await this.refreshFriendList();
  },
