...
class PortableChat {
  ...
  async _loginFlow() {
    for (;;) {
      ...
      if (ipUsers.length > 0) {
        lines.push('3. 本机已有账号:');
        ipUsers.forEach((u) => lines.push(`     #${u.id} ${u.username} (${u.ip}-${u.ip_index || '?'})`));
      }
      lines.push('0. 退出');
  ...
  async _aiSetup() {
    let status = null;
    try {
      status = await this.client.getAiStatus();
    } catch {
      this._print('无法连接服务器');
      return;
    }
    ...
    if (status.account) {
      this._print(`AI 助理已注册：${status.account.username} (#${status.account.id})`);
      return;
    }
    ...
    this._print(`✓ AI 助理已注册：${res.user.username} (#${res.user.id})，已自动加为好友`);
  }
