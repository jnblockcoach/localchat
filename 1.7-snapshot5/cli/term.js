...
class TerminalUI {
  ...
  _drawHeader() {
    if (this._uiMode === 'requests' || this._uiMode === 'ai') return;
  ...
  _renderRequestsView(errMsg) {
    ...
    if (this._reqList.length === 0) {
      this._put(cx + 2, cy + 4, ' 暂无好友请求，按 ESC 返回', 'gray');
    } else {
  ...
  async _showAiView() {
    try {
      const res = await this.client.getAiStatus();
      this._aiStatus = res;
    } catch {
  ...
  _renderAiView(errMsg) {
    ...
    const cy = Math.max(0, Math.floor((H - 14) / 2));
    this._box(cx, cy, 56, 14);
    this._put(cx + 2, cy + 1, ' AI 助理   [Enter]注册  [V]Workspace  [ESC]返回', 'brightCyan');
    this._put(cx + 2, cy + 2, '\u2500'.repeat(52), 'gray');
    ...
    if (acc) {
      this._put(cx + 2, cy + 7, ' 按 V 查看 AI 的 Workspace', 'gray');
    }
    this._put(cx + 2, cy + 13, errMsg ? '⚠ ' + errMsg : ' 按 Enter 注册  V 查看Workspace  ESC 返回', errMsg ? 'red' : 'gray');
  }
  ...
  _renderWsView(errMsg) {
    ...
    const cy = Math.max(0, Math.floor((H - 14) / 2));
    this._box(cx, cy, 56, 14);
    this._put(cx + 2, cy + 1, ' AI Workspace   ↑↓选择  Enter查看  B返回列表  ESC返回', 'brightCyan');
    this._put(cx + 2, cy + 2, '\u2500'.repeat(52), 'gray');
  ...
  async _loadWsFiles() {
    try {
      const res = await this.client.getAiWorkspace();
    ...
    this._wsView = 'list';
    this._wsContent = '';
    this._renderWsView(res && res.error ? res.error : undefined);
  }
  ...
  async _handleWsKey(name) {
    ...
    if (name === 'ENTER') {
      ...
      if (!f) return;
      try {
        const res = await this.client.getAiWorkspace(f.name);
  ...
  _doEsc() {
    if (this._uiMode === 'requests' || this._uiMode === 'ai') {
